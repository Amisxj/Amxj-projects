...
if (!function_exists('allow_release_arctype')) 
{
     ...
     * 允许发布文档的栏目列表
     */
    function allow_release_arctype($selected = 0, $allow_release_channel = array(), $selectform = true, $release_typeids = [])
    {
        $where = [];
        ...
        if (empty($select_html) || false == $selectform) {
            ...
            if (false == $selectform) {
                ...
                handle_arctype_data($select_html, $nowArr, $selected, $allow_release_channel);
...
{
    // 处理栏目数据
    function handle_arctype_data(&$select_html = '', $nowArr = [], $selected = 0, $allow_release_channel = [])
    {
        foreach ($nowArr AS $key => $val)
        {
            ...
            handle_arctype_data($select_html, $val['children'], $selected, $allow_release_channel);
        }
    }
}
    ...
    function site_seo_handle($value = '', $site_info = [])
    {
        if (!empty($value)) {
            ...
            if (stristr($value, "{region}")) {
                $value = str_ireplace('{region}', $site_info['name'], $value);
            }
